...
namespace ac {
	...
	struct dynamicEntity {
		...
		heldItemStyle heldStyle = heldItemStyle::none;
		DirectX::XMFLOAT4X4 heldItemWorld{};
		DirectX::XMFLOAT4X4 viewmodelWorld{};
		bool heldItemReady = false;
		bool viewmodelReady = false;
	};
	...
	public:
		...
		bool triggerAttackAnimation(dynamicEntityHandle handle) {
			dynamicEntity* entity = get(handle);
			if (!entity || entity->type != dynamicEntityType::humanoid || !_humanoidAsset)
				return false;
			_humanoidAsset->triggerAttackAnimation(entity->humanoidAnimation, entity->heldStyle);
			return true;
		}
		...
		bool triggerUseAnimation(dynamicEntityHandle handle) {
			...
			if (!entity || entity->type != dynamicEntityType::humanoid || !_humanoidAsset)
				return false;
			_humanoidAsset->triggerUseAnimation(entity->humanoidAnimation, entity->heldStyle);
			return true;
		}
		...
		dynamicRenderStats renderHumanoids(
			...
			for (entitySlot& slot : _entities) {
				...
				++stats.submitted;
				entity.viewmodelReady = false;
				entity.heldItemReady = false;
				if (!entity.visible) {
					++stats.culled;
					continue;
				}
				...
				if (entity.thirdPerson && entity.maximumDrawDistance > 0.0f && distance > limit * limit) {
					++stats.culled;
					continue;
				}
				_humanoidAsset->render(
					...
					entity.humanoidAnimation,
					entity.heldStyle,
					entity.heldStyle == heldItemStyle::none ? nullptr : &entity.heldItemWorld
				);
				entity.heldItemReady = entity.heldStyle != heldItemStyle::none;
				entity.viewmodelReady = false;
				++stats.drawn;
			}
		...
		bool drawFirstPersonViewmodel(dynamicEntityHandle handle) {
			dynamicEntity* entity = get(handle);
			if (!_humanoidAsset || !entity || !entity->viewmodelReady) return false;
			_humanoidAsset->drawViewmodel(_context, entity->viewmodelWorld);
			return true;
		}
